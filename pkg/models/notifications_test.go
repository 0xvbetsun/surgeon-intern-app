...
func testNotificationToOneUserUsingUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUser(ctx, tx, false, (*[]*Notification)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if local.R.User == nil {
...
func testNotificationToOneSetOpUserUsingUser(t *testing.T) {
	var err error
...
var (
	notificationDBTypes = map[string]string{`ID`: `uuid`, `Annotations`: `jsonb`, `UserID`: `uuid`, `CreatedAt`: `timestamp with time zone`, `SeenAt`: `timestamp with time zone`}
	_                   = bytes.MinRead
)
