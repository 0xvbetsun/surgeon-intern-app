...
var (
	roleAllColumns            = []string{"id", "name", "display_name"}
	roleColumnsWithoutDefault = []string{"name"}
	roleColumnsWithDefault    = []string{"id", "display_name"}
...
func (o *Role) UserOrganizationalUnitRoles(mods ...qm.QueryMod) userOrganizationalUnitRoleQuery {
	...
	queryMods = append(queryMods,
		qm.Where("\"user_organizational_unit_roles\".\"role_id\"=?", o.ID),
	)
...
func (roleL) LoadUserOrganizationalUnitRoles(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		...
		qm.WhereIn(`user_organizational_unit_roles.role_id in ?`, args...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *Role) AddUserOrganizationalUnitRoles(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*UserOrganizationalUnitRole) error {
	var err error
