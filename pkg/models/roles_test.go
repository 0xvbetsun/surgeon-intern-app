...
func testRoleToManyUserOrganizationalUnitRoles(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadUserOrganizationalUnitRoles(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpUserOrganizationalUnitRoles(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `integer`, `Name`: `character varying`, `DisplayName`: `character varying`}
	_           = bytes.MinRead
)
